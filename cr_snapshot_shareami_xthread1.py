...
def snapvolumes_task(volume):
    print "starting snapvolumes_task thread of " + volume.id + " at " + str(datetime.datetime.now())
    target_session = role_arn_to_session(
        RoleArn=ROLE_ON_TARGET_ACCOUNT,
        RoleSessionName='share-admin-temp-session',
        DurationSeconds = 40000
    ...
    if source_sharing['CreateVolumePermissions'] \
        ...
        print("Snapshot already shared with account, creating a copy")
         ...
         print "Sharing with target account volume " + volume.id
    ...
    try:
        print "waiting for snapshot to complete on target 1st time"
        ...
        try:
            print "waiting for snapshot to complete target for target encryption 2nd time "
            waitert_snapshot_complete.wait(
                SnapshotIds=[
                    copied_snapshot.id,
                ]
    ...
    print "snapvolumes_task  thread of volume " + volume.id + "  completed  at " + str(datetime.datetime.now())
