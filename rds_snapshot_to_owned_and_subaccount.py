...
class DBSnapshot(object):
    """DBSnapshot"""
    ## CMK of root account
    ###cmkid='f752057c-0c05-47b3-ab8e-251bffd85d4d'
    ##cmkid of itadmin account
    cmkid='2c9e26fc-3059-4eb3-8e82-a32813195943'
    #
    ## CMK of target account
    ...
    def create(db_instance, timestamp):
        ...
        DBSnapshot.snapshot_id = "{0}-{1}-{2}".format("testinstance6a", db_instance, timestamp)
    ...
    @classmethod
    def restore_db_snapshot(cls,dbsnapshotidentifier,dbsubnetgroupname,dbinstanceclass,availabilityzone,dbparametergroupname,vpcsecuritygroupids):
...
def main():
    ...
    parser.add_argument('--restore_own_account', help="Restore db snapshot to own acccount  ", action='store_true')
    ...
    #DBSnapshot.snapcopy_enc_id='mysnapshot-testinstance-2018-12-05-2nd-encrypt'

    DBSnapshot.snapcopy_enc_id_arn=DBSnapshot.get_snapshot_arn(DBSnapshot.snapcopy_enc_id)
    ...
    ##to be deletd
    #DBSnapshot.snapcopy_enc_id='arn:aws:rds:us-east-1:006775277657:snapshot:mysnapshot-testinstance-2018-12-05-2nd-encrypt'
    #DBSnapshot.snapshot_id='mysnapshot-testinstance-2018-12-05'

    #snapsource_arn=DBSnapshot.get_snapshot_arn(DBSnapshot.snapcopy_enc_id_arn)
    DBSnapshot.snapcopy_enc_sub_id=DBSnapshot.copy_db_snapshot(DBSnapshot.snapcopy_enc_id_arn,"-subaccount-encrypt",True)
    ...
    if restore_sub_account:
        DBSnapshot.restore_db_snapshot(DBSnapshot.snapcopy_enc_sub_id,dbsubnetgroupname,dbinstanceclass,availabilityzone,dbparametergroupname,vpcsecuritygroupids)
    ...
    if args.restore_own_account:
        answer = raw_input("Please type YES to restore database from encrypted snapshot - Make sure original DB has been renamed : ")
        if answer=='YES':

                #dbsubnetgroupname='subnet_group_cloudify'
                #dbsubnetgroupname='subnet_group_cloudify'
                dbsubnetgroupname='corepoint-dev-subnet'
                dbinstanceclass='db.r3.xlarge'
                availabilityzone='us-east-1a'
                dbparametergroupname='default.mysql5.6'
                #dbparametergroupname='default.sqlserver-se-12.00'
                vpcsecuritygroupids=['sg-08c7c88f325a2eef6']
                DBSnapshot.initialize()
                DBSnapshot.restore_db_snapshot(DBSnapshot.snapcopy_enc_id,dbsubnetgroupname,dbinstanceclass,availabilityzone,dbparametergroupname,vpcsecuritygroupids)
...
##c:\Python27\python.exe rds_snapshot_to_subaccount.py share-db-snapshot --db_identifier corepoint-mssql --snapshotid mysnapshot-testinstance-2018-12-04copy-enc --other_sub_account 417302553802 --restore_sub_account