...
def modify_instance_dbid(db, newdbid, instance_class=None):
    """given snapshot to configured db instance name"""
    if not instance_class:
        instance_class = config.get('DEFAULT', INSTANCE_CLASS)

    sys.stdout.write(
        'Modifying Database Identifier  %s to instance ID  %s, confirm with YES (all capitals)\n'
        % (db, newdbid))
    choice = raw_input()

    if choice == 'YES':
        print('Modify Db Idenfier from %s to %s' % (db, newdbid))

        rds_client = boto3.client('rds')
        resp=rds_client.modify_db_instance(
            DBInstanceIdentifier=db,
            NewDBInstanceIdentifier=newdbid,
...
def main():
    ...
    parser.add_argument('command',
                        type=str,
                       choices=[COMMAND_RESTORE, COMMAND_DELETE, COMMAND_MODIFY])
    parser.add_argument('--db-identifier',
                        help='db_instance_identifier (maschine name)',
                        type=str)
    parser.add_argument('--new-dbid', help="To Modify Database Identifer", action='store',dest='newdbid')
    parser.add_argument('--snapshotid', help="To Restore using a particular  Snapshot Identifier ", action='store')
    # parse args
    ...
    if command == COMMAND_RESTORE:
        ...
        if not args.snapshotid:
            ...
            snapshot_identifier = args.snapshotid
        ...
        modify_instance_dbid(db_identifier,args.newdbid)
