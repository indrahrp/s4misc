...
stack = core.Stack(app, "VONC-VISTA", env={'region': 'us-east-1','account':'921279086507'})
...
task_definition_varsleuth = ecs.Ec2TaskDefinition(
    stack, "VONC_VISTA-task-varsleuth",
    network_mode=ecs.NetworkMode.AWS_VPC,
...
task_definition_dispatcher = ecs.Ec2TaskDefinition(
    stack, "VONC_VISTA-task-dispatcher",
    network_mode=ecs.NetworkMode.AWS_VPC,
...
task_definition_spliceai = ecs.Ec2TaskDefinition(
    stack, "VONC_VISTA-task-spliceai",
    network_mode=ecs.NetworkMode.AWS_VPC,
    volumes=[docker_volume],
    family='VONC_VISTA-Spliceai'
)
...
voncweb_container = task_definition_voncweb.add_container(
    "VONC_VISTA-vonc-container",
    image=ecs.ContainerImage.from_registry(ecr_repo_voncweb),
    cpu=16,
    memory_limit_mib=256,
    essential=True,
    logging=ecs.LogDriver.aws_logs(stream_prefix="VONC_VISTA_vonc_logs"),
    privileged=True,
...
dispatcher_container = task_definition_dispatcher.add_container(
    "VONC_VISTA-dispatcher-container",
    image=ecs.ContainerImage.from_registry(ecr_repo_dispatcher),
    cpu=16,
    memory_limit_mib=256,
    essential=True,
    logging=ecs.LogDriver.aws_logs(stream_prefix="VONC_VISTA_dispatcher_logs"),
    privileged=True,
    #stop_timeout=core.Duration.hours(8)
...
varsleuth_container = task_definition_varsleuth.add_container(
    "VONC_VISTA-varsleuth-container",
    image=ecs.ContainerImage.from_registry(ecr_repo_varsleuth),
    cpu=16,
    memory_limit_mib=256,
    essential=True,
    logging=ecs.LogDriver.aws_logs(stream_prefix="VONC_VISTA_varsleuth_logs"),
    privileged=True,
    #stop_timeout=core.Duration.hours(8)
    #environment= {'HG19':'/var/www/html/mydo/hg19/ucsc.hg19.fasta','refSeqVer':'GRCH37','MYDO_DIR':'/var/www/html/mydo/LAGI','TMPDIR':'/tmp'}
...
spliceai_container = task_definition_spliceai.add_container(
    "VONC_VISTA-spliceai-container",
    image=ecs.ContainerImage.from_registry(ecr_repo_varsleuth),
    cpu=16,
    memory_limit_mib=256,
    essential=True,
    logging=ecs.LogDriver.aws_logs(stream_prefix="VONC_VISTA_spliceai_logs"),
    privileged=True,
    #stop_timeout=core.Duration.hours(8)
    #environment= {'HG19':'/var/www/html/mydo/hg19/ucsc.hg19.fasta','refSeqVer':'GRCH37','MYDO_DIR':'/var/www/html/mydo/LAGI','TMPDIR':'/tmp'}
...
service_varsleuth = ecs.Ec2Service(
    stack, "VONC_VISTA-ecs-service-varsleuth",
    cluster=cluster,
    ...
    desired_count=1,
    security_group=security_group,
    service_name='VONC_VISTA-VarSleuth'

)


service_spliceai = ecs.Ec2Service(
    stack, "VONC_VISTA-ecs-service-spliceai",
    cluster=cluster,
    ...
    desired_count=1,
    security_group=security_group,
    service_name='VONC_VISTA-SpliceAI'
...
lb_voncweb = elbv2.ApplicationLoadBalancer(
    stack, "VONC_VISTA_lb_voncweb",
    load_balancer_name='VONC-VISTA-Vonc-ELB',
    vpc=vpc,
    internet_facing=True
)

lb_dispatcher = elbv2.ApplicationLoadBalancer(
    stack, "VONC_VISTA_lb_dispatcher",
    load_balancer_name='VONC-VISTA--Dispatcher-ELB',
    vpc=vpc,
    internet_facing=True
)
...
lb_vista = elbv2.ApplicationLoadBalancer(
    ...
    load_balancer_name='VONC-VISTA-Vista-ELB',
    vpc=vpc,
    internet_facing=True
)
...
lb_varsleuth = elbv2.ApplicationLoadBalancer(
    stack, "VONC_VISTA_lb_varleuth",
    vpc=vpc,
    load_balancer_name='VONC-VISTA-Varsleuth-ELB',
    internet_facing=True
)
...
lb_spliceai = elbv2.ApplicationLoadBalancer(
    stack, "VONC_VISTA_lb_spliceai",
    vpc=vpc,
    load_balancer_name='VONC-VISTA-SpliceAI-ELB',
    internet_facing=True
)
...
listener_voncweb.add_targets(
    "voncweb",
    target_group_name='VONC-VISTA-VoncWeb-Blue',
    port=443,
...
listener_dispatcher.add_targets(
    "dispatcher",
    target_group_name='VONC-VISTA-Dispatcher-Blue',
    port=3000,
...
listener_vista.add_targets(
    "vistaweb_target",
    target_group_name='VONC-VISTA-Vista-Blue',
    port=80,
...
listener_varsleuth.add_targets(
    "varsleuth_target",
    target_group_name='VONC-VISTA-Varsleuth-Blue',
    port=80,
...
listener_spliceai.add_targets(
    "spliceai_target",
    target_group_name='VONC-VISTA-SpliceAI-Blue',
    port=80,
...
voncweb_target_green=elbv2.ApplicationTargetGroup(stack,"voncweb_target_green",target_group_name='VONC-VISTA-VoncWeb-Green',port=443,target_type=elbv2.TargetType.IP,health_check=health_check,vpc=vpc)
listener_voncweb_green.add_target_groups("voncweb_green",target_groups=[voncweb_target_green])


dispatcher_target_green=elbv2.ApplicationTargetGroup(stack,"dispatcher_target_green",target_group_name='VONC-VISTA-Dispatcher-Green',port=3000,protocol=elbv2.ApplicationProtocol.HTTP,target_type=elbv2.TargetType.IP,health_check=dispatcher_health_check,vpc=vpc)
listener_dispatcher_green.add_target_groups("dispatcher_green",target_groups=[dispatcher_target_green])

vistaweb_target_green=elbv2.ApplicationTargetGroup(stack,"vistaweb_target_green",port=80,target_group_name='VONC-VISTA-Vista-Green',protocol=elbv2.ApplicationProtocol.HTTP,target_type=elbv2.TargetType.IP,health_check=health_check,vpc=vpc)
listener_vista_green.add_target_groups("vistaweb_green",target_groups=[vistaweb_target_green])


varsleuth_target_green=elbv2.ApplicationTargetGroup(stack,"varsleuth_target_green",port=80,target_group_name='VONC-VISTA-Varsleuth-Green',protocol=elbv2.ApplicationProtocol.HTTP,target_type=elbv2.TargetType.IP,health_check=health_check,vpc=vpc)
listener_varsleuth_green.add_target_groups("varsleuth_green",target_groups=[varsleuth_target_green])



spliceai_target_green=elbv2.ApplicationTargetGroup(stack,"spliceai_target_green",port=80,target_group_name='VONC-VISTA-SpliceAI-Green',protocol=elbv2.ApplicationProtocol.HTTP,target_type=elbv2.TargetType.IP,health_check=health_check,vpc=vpc)
...
core.CfnOutput(
    stack, "VONC_VISTA-LoadBalancerDNS_voncweb",
    value=lb_voncweb.load_balancer_dns_name
)

core.CfnOutput(
    stack, "VONC_VISTA-LoadBalancerDNS_dispatcher",
    value=lb_dispatcher.load_balancer_dns_name
)


core.CfnOutput(
    stack, "VONC_VISTA-LoadBalancerDNS_vista",
    value=lb_vista.load_balancer_dns_name
)

core.CfnOutput(
    stack, "VONC_VISTA=LoadBalancerDNS_varsleuth",
    value=lb_varsleuth.load_balancer_dns_name
)


core.CfnOutput(
    stack, "VONC_VISTA=LoadBalancerDNS_spliceai",
    value=lb_varsleuth.load_balancer_dns_name
)
